...
@section('content')
    ...
    <ul class="breadcrumb mt-2">
        ...
        <li class="breadcrumb-item" style="font-weight: bold; color: #009688">{{__('Branches ')}}( {{$company->name}})</li>
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-12">
                        @if (auth()->user()->hasPermission('create_branches'))
                            <a href="{{url('admin/companies/'.$company->id.'/branches/create')}}" class="btn btn-primary"><i class="fa fa-plus"></i>
                                {{__('Create')}}</a>
                        @endif

                        @if (auth()->user()->hasPermission('delete_branches'))
                            <form method="post" action="{{ route('admin.branches.bulk_delete') }}" style="display: inline-block;">
                                @csrf
                                @method('delete')
...
@push('scripts')
    ...
    <script>
        ...
        let branchesTableTable = $('#branchesTable-table').DataTable({
            ...
            ajax: {
                url: '{{ route('admin.branches.data',$company->id) }}',
            },
            columns: [
                {data: 'record_select', name: 'record_select', searchable: false, sortable: false, width: '1%'},
                {data: 'name', name: 'name'},
                {data: 'created_at', name: 'created_at', searchable: false},
                {data: 'add_job_title', name: 'add_job_title',searchable: false , sortable: false},
